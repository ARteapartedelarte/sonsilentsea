...
class Widget:
	...
	def __init__(self, parent, name, aspect=None, size=[0, 0], pos=[0, 0], sub_theme='',
		...
		self._rotation = 0.0
		self._update_position(size, pos, 0.0)
	...
	def _rotatePoint(self, point, center, angle):
		p = point[:]
		p = [p[j] - center[j] for j in [0,1]]
		r = [p[0]*cos(angle) - p[1]*sin(angle),
		     p[0]*sin(angle) + p[1]*cos(angle)]
		return [r[j] + center[j] for j in [0,1]]

	def _rotate(self, center, angle):
		for i in range(4):
			self.gl_position[i] = self._rotatePoint(self.gl_position[i], center, angle)
		# Apply the rotation to the childrens
		for widget in self.children.values():
			widget._rotate(center, angle)
	...
	def position(self, value):
		self._update_position(self._base_size, value, self._base_rot)
	...
	@size.setter
	def size(self, value):
		self._update_position(value, self._base_pos, self._base_rot)

	@property
	def rotation(self):
		"""The widget's rotation (in radians)"""
		return self._rotation

	@rotation.setter
	def rotation(self, value):
		self._update_position(self._base_size, self._base_pos, value)
	...
	def _handle_mouse(self, pos, event):
		...
		for widget in self.children.values():
			...
			diag01 = [widget.gl_position[1][0] - widget.gl_position[0][0],
			          widget.gl_position[1][1] - widget.gl_position[0][1]]
			diag12 = [widget.gl_position[2][0] - widget.gl_position[1][0],
			          widget.gl_position[2][1] - widget.gl_position[1][1]]
			p0     = [pos[0] - widget.gl_position[0][0],
			          pos[1] - widget.gl_position[0][1]]
			p1     = [pos[0] - widget.gl_position[1][0],
			          pos[1] - widget.gl_position[1][1]]
			p2     = [pos[0] - widget.gl_position[2][0],
			          pos[1] - widget.gl_position[2][1]]
			def dot(a,b):
				return a[0]*b[0] + a[1]*b[1]
			if (dot(p0,diag01)*dot(p1,diag01) <= 0.0) and \
			   (dot(p1,diag12)*dot(p2,diag12) <= 0.0):
				widget._handle_mouse(pos, event)
			else:
				widget._update_hover(False)
