...
class TextInput(Widget):
	...
	def __init__(self, parent, name, text="", prefix="", font=None, pt_size=None, color=None,
		...
		# The top parent, used to can take into account rotations over the cursor and highlight
		self._topParent = self.parent
		while self._topParent != self._topParent.parent:
			self._topParent = self._topParent.parent
	...
	def update_selection(self):
		...
		# To compute all parent rotations the most easy way is calling to update the top parent
		self._topParent._update_position()
	...
	def calc_mouse_cursor(self, pos):
		...
		# Get the normalized guide vector of the text box
		guide   = [self.gl_position[1][0] - self.gl_position[0][0],
		           self.gl_position[1][1] - self.gl_position[0][1]]
		lguide  = length(guide)
		if not lguide:
			lguide = 1.0
		guide   = [guide[i]/lguide for i in [0,1]]
		# Get the relative position of the mouse with respect the text box
		adj_pos = [pos[0] - self.gl_position[0][0],
		           pos[1] - self.gl_position[0][1]]
		# Project the position over the guide to compute the distance in the text space
		adj_pos = dot(adj_pos,guide) - self.fd
